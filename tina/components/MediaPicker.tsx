...
const isPickerProps = (value: MediaPickerProps): value is MediaPickerBaseProps & MediaPickerPickerModeProps => {
  return value.mode !== "manager";
};
...
export const MediaPicker = (props: MediaPickerProps) => {
            ...
            >
              {items.map((item) => {
                ...
                return (
                  <div
                    key={item.id}
                    onClick={() => bulkDeleteMode ? toggleBulkDelete(item.src) : mode === "picker" ? toggleSelection(item.src) : undefined}
  ...
  if (embedded) {
    return <div className="w-full h-full flex flex-col bg-gray-50">{mediaContent}</div>;
  }
