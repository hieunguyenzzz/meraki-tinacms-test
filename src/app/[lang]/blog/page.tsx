                ...
                <div className="mb-16">
                  ...
                  <article className="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    <div className="md:flex">
                      ...
                      <div className={`p-8 ${blogs[0].featured_image ? 'md:w-1/2' : 'w-full'}`}>
                        {blogs[0].categories && blogs[0].categories.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-4">
                            {blogs[0].categories.slice(0, 3).map((category: string, idx: number) => (
                              <span 
                                key={idx}
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-gray-500">
                            {blogs[0].published_date 
                              ? new Date(blogs[0].published_date).toLocaleDateString(lang === 'en' ? 'en-US' : 'vi-VN')
                              : new Date(blogs[0]._sys.createdAt).toLocaleDateString(lang === 'en' ? 'en-US' : 'vi-VN')
                            }
                          </span>
                          <Link 
                            href={`/${lang}/blog/${blogs[0].slug || blogs[0]._sys.filename.replace('.mdx', '')}`}
                          ...
                          <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-500">
                              {blog.published_date 
                                ...
                                : new Date(blog._sys.createdAt).toLocaleDateString(lang === 'en' ? 'en-US' : 'vi-VN')
                              }
                            </span>
                            <Link 
                              href={`/${lang}/blog/${blog.slug || blog._sys.filename.replace('.mdx', '')}`}
